  ...
  paste(
    ...
    c(                    # remove panel id's from OLS model
      ...
      names(analysis)[ names(analysis) %in% c(
        ...
        'regime_type', 'SI.POV.DDAY', 'absolute_poverty'
      ) == FALSE]
    ), 
    collapse = ' + '
    )
...
compareCoefs(fit1, fit2) # strong increase in beta_[d_military]
...
plot(c(-44, 20), c(-10, 10), type = 'n')
curve(coef_t_fit2[1] + coef_t_fit2[2] * x + coef_t_fit2[3] * x^2 + coef_t_fit2[4] * x^3/1000, col = 'blue', add = TRUE)
curve(coef_t_fit3[1] + coef_t_fit3[2] * x + coef_t_fit3[3] * x^2 + coef_t_fit3[4] * x^3/1000, col = 'red', add = TRUE)
...
sim_results <- gather(sim_results, 'sim_no', 'value', 17:ncol(sim_results))
str(sim_results)
